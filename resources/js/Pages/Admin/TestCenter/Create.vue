...
async function submit() {
  ...
  try {
    ...
    const res = await window.axios.post('/admin/test_center/save', fd, {
    ...
    window.location.href = '/admin/test_center'
  } catch (err) {
...
<template>
  <Head title="นำเข้าศูนย์สอบ (Excel)" />

  <AdminLayout>
    <div class="space-y-6">
      <div>
        <div class="text-xs font-semibold tracking-[0.18em] text-slate-300/60">IMPORT</div>
        <h1 class="mt-1 text-2xl font-semibold">นำเข้าศูนย์สอบ (Excel)</h1>
          ...
          <div class="lg:col-span-1">
            <div class="rounded-xl border border-white/10 bg-slate-950/30 p-5">
              ...
              <div class="mt-3 space-y-2 text-xs text-slate-300/70">
                <div>- ระบบจะนำเข้าไฟล์ตามลำดับเลขหน้าชื่อไฟล์ (ถ้ามี)</div>
                <div>- เมื่อนำเข้าสำเร็จจะกลับไปหน้า “ศูนย์สอบ”</div>
              </div>
            ...
            <!-- keep csrf in DOM for classic form parity if needed -->
