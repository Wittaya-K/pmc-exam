...
class ArrangeSeatController extends Controller
{
    ...
    //     ->where('test_center', '=', 'คณะวิทยาศาสตร์ม.อ.วิทยาเขตหาดใหญ่')
    //     ->groupBy('program_name')
    //     ->orderBy('program_name', 'asc') // เรียงเหมือน $students
    //     ->pluck('program_name');
    ...
    //     $p46_total = $programCounts[$programA] ?? 0;
    //     $m13_total = $programCounts[$programB] ?? 0;
    //     $m46_total = $programCounts[$programC] ?? 0;
    ...
    //         else if ($program === $programB) {
    //             if ($m13_count < $m13_to_A) {
    //                 $programGroups["{$programB} → Session A"][] = $student;
    //             } else {
    //                 $programGroups["{$programB} → Session B"][] = $student;
    //             }
    //             $m13_count++;
    //         }
    //         else if ($program === $programC) {
